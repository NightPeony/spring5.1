...
public class ClassPathXmlApplicationContext extends AbstractXmlApplicationContext {
	...
	public ClassPathXmlApplicationContext(
			...
			throws BeansException {
		//这设置父容器  因该想起mvc前端核心控制器也是初始化一个容器  那个容器创建的时候会将父容器设置进去，不是set，而是构造方法像这样直接构造进去
		//所以service层注入controller是注入不进去的  一般也没人这么做
		//这里记住  设置父容器  提取父容器开发环境
		super(parent);
		setConfigLocations(configLocations);
		if (refresh) {
			//所有刷新不设置懒加载的bean的是基本都在这  这个方法是父类的
			refresh();
		}
	}
